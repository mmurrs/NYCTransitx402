...
# Resolve repo root from this script's location — works for any cloner.
...
# Verify x402 payload decodes correctly. Expected network is read from the
# same env the server booted with, so mainnet and testnet configs both work.
EXPECTED_NETWORK="$(grep -E '^X402_NETWORK=' "$ENV_FILE" | tail -n1 | cut -d= -f2- | tr -d '"' | tr -d "'")"
EXPECTED_NETWORK="${EXPECTED_NETWORK:-eip155:8453}"

X402_DECODED=$(echo "$HEADERS" | grep -i "payment-required:" | sed 's/.*: //' | tr -d '\r' | base64 -d 2>/dev/null || echo "")
if echo "$X402_DECODED" | EXPECTED_NETWORK="$EXPECTED_NETWORK" python3 -c "
import os, sys, json
d = json.load(sys.stdin)
assert d['x402Version'] == 2, f'x402Version={d.get(\"x402Version\")}'
got = d['accepts'][0]['network']
want = os.environ['EXPECTED_NETWORK']
assert got == want, f'network: got {got}, expected {want}'
" 2>/dev/null; then
  pass "x402 payload: valid v2, network=$EXPECTED_NETWORK"
else
  fail "x402 payload: decode or validation failed (expected network=$EXPECTED_NETWORK)"
fi
