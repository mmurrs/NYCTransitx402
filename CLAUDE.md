...
Public repo for open-source deployment.

## Architecture
- `server.js` — Express entry point, imports `dual402.js` for payment middleware
- `dual402.js` — Dual payment middleware accepting both x402 (Base mainnet) and MPP (Tempo USDC)
...
## Deployment
- **Backend**: Deploy to EigenCloud or any Node.js host
  - EigenCloud: `ecloud compute app upgrade <YOUR_APP_ID> --environment sepolia --env-file .env`
  - IMPORTANT: `--env-file` flag is buggy — use `export ECLOUD_ENVFILE_PATH=...` env var or ensure the command is on ONE line
...
- **Vercel** (proxy/landing page): `cd proxy && npx vercel --prod`
  - Set `BACKEND_URL` env var in Vercel to point to your backend server IP

## Branding
- Brand: **NYC Transit Live** — transit modes are Bike (#0073CE), Subway (#FF6319), Bus (#00ADD0)
